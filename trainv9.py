...
#---v8obb --lm
from models.talv8 import *
from models.headv8 import *
from models.lossv8 import *
...
# import val_v5lm as validate  # for end-of-epoch mAP
...
# from utils.datasets_lmxz import create_dataloader
...
from utils.loss_tal import ComputeLoss, v8DetectionLoss, v10DetectLoss, ComputeLoss_dual
# from models.lossv8 import v8DetectionLoss, v10DetectLoss
from models.yolox_loss import ComputeXLoss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), sr=opt.super, factor=down_factor).to(device)  # create
    ...
    # compute_loss=v8OBBLoss(model)
    compute_loss = ComputeLoss_dual(model)  # init loss class   # gai--lm  
    # compute_loss = v8DetectionLoss(model)  # init loss class   # gai--lm  
    # compute_loss = v10DetectLoss(model)  # init loss class   # gai--lm     
    # compute_loss = ComputeXLoss(model)  # init loss class   #gai-----***********lm*************----
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.super:
            LOGGER.info(('\n' + '%11s' * 8) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss', 'SRloss', 'Instances', 'Size'))    #gai--lm
        else:
            LOGGER.info(('\n' + '%11s' * 7) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss',  'Instances', 'Size'))    #gai--lm
        if RANK in {-1, 0}:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #zjq  jia
            if down_factor>1:
                imgs00=F.interpolate(imgs,size=[i//down_factor for i in imgs.size()[2:]], mode='bilinear', align_corners=True)
            else:
                imgs00 = imgs
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if opt.super:  #
                    pred, output_sr = model(imgs)  # forward # lm    return x, output_sr
                else:
                    pred = model(imgs)
                ...
                if opt.super:  #
                    sr_loss = 2 * torch.nn.L1Loss()(output_sr, imgs00)
                    loss = loss + sr_loss  # ****
            ...
            if RANK in {-1, 0}:
                ...
                if opt.super:
                    pbar.set_description(('%11s' * 2 + '%11.4g' * 6) %  # gai  --lm 5->6
                                         (f'{epoch}/{epochs - 1}', mem, *mloss, sr_loss, targets.shape[0],
                                          imgs.shape[-1]))  # jia loss lm
                else:
                    pbar.set_description(('%11s' * 2 + '%11.4g' * 5) %  # gai  --lm 5->6
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT /'models/detect/v9/yolov9-s.yaml', help='model.yaml path')
    ...
    # parser.add_argument('--project', default=ROOT / 'runs/train/v9', help='save to project/name')
