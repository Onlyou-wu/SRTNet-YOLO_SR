...
from utils.tal.assigner import TaskAlignedAssigner, TaskAlignedAssignerv8
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        # 调试：打印feats中每个元素的类型
        # 调试：打印feats的整体结构
        print("feats整体类型：", type(feats))
        print("feats元素数量：", len(feats))
        print("feats元素类型：", [type(xi) for xi in feats])

        # 从嵌套列表中提取张量（关键修复）
        valid_feats = []
        for outer_item in feats:
            # 检查外层元素是否为列表或元组
            if isinstance(outer_item, (list, tuple)):
                # 遍历内层元素，寻找张量
                for inner_item in outer_item:
                    if isinstance(inner_item, torch.Tensor):
                        valid_feats.append(inner_item)
                        break  # 找到第一个张量后退出内层循环
            # 如果外层元素本身就是张量，直接添加
            elif isinstance(outer_item, torch.Tensor):
                valid_feats.append(outer_item)

        # 验证提取结果
        if not valid_feats:
            raise ValueError("未从feats中提取到任何张量，请检查网络输出结构")
        print("提取的有效张量数量：", len(valid_feats))
        print("有效张量形状：", [t.shape for t in valid_feats])
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.assigner = TaskAlignedAssignerv8(topk=tal_topk, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLossv8(m.reg_max - 1, use_dfl=self.use_dfl).to(device)
    ...
    def __call__(self, preds, batch):
        ...
        loss[0] *= self.hyp['box']  # 用键索引访问字典元素
        loss[1] *= self.hyp['cls']  # 用键索引访问字典元素
        loss[2] *= self.hyp['dfl']  # 用键索引访问字典元素
        # loss[0] *= self.hyp.box  # box gain   7.5
        # loss[1] *= self.hyp.cls  # cls gain   0.0125
