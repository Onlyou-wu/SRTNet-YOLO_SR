...
def xywhn2xyxylm(x, w=640, h=640, padw=0, padh=0):
...
def xyn2xylm(x, w=640, h=640, padw=0, padh=0):
    # Convert normalized segments into pixel segments, shape (n,2)
    #将归一化段转换为像素段，形状为 (n,2)
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = x[:, 0] + padw  # top left x
...
def scale_coordslm(img1_shape, coords, img0_shape, ratio_pad=None):
    # Rescale coords (xyxy) from img1_shape to img0_shape  将坐标 (xyxy) 从 img1_shape 重新缩放为 img0_shape
    if ratio_pad is None: #不进去    # calculate from img0_shape  从 img0_shape 计算
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    else:
        gain = ratio_pad[0][0] # 1.0
        pad = ratio_pad[1]    #【16.0，16.0】
...
def non_max_suppression(
    ...
    if isinstance(prediction, list) and len(prediction) > 0:   #gai --lm
        device = prediction[0].device  # 访问列表中的第一个元素的device属性
        prediction = prediction[0]  # select only inference output
    else:
...
def non_max_suppressionv5(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 6:] > conf_thres).nonzero(as_tuple=False).T    #gai--lm
            x = torch.cat((box[i], x[i, j + 6, None], j[:, None].float()), 1)    #gai--lm
        else:  # best class only
            conf, j = x[:, 6:].max(1, keepdim=True)   #gai
            x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]   #>0.35
        ...
        if not n:  # no boxes
            continue
        elif n > max_nms:  # excess boxes  flase
            x = x[x[:, 5].argsort(descending=True)[:max_nms]]  # sort by confidence    #gai--lm
        ...
        # 2选1屏蔽
        # BBX = xywh2xyxy(x[:, :4])   + c    #  xywh-->xyxy   水平框nms
        # i0 = torchvision.ops.nms(BBX, scores, iou_thres)  # NMS  水平框nms
        #
        # xxt = x[i0]
        # Rboxes=xxt[:, :5]
        # R_scores = xxt[:, 5]  # boxes (offset by class), scores

        from .ops import nms_rotated
        x_lm = x.clone()  # 保留原来数据进行接下来的操作

        boxes[:, :2] = boxes[:, :2] + c   # 旋转框nms
        i = nms_rotated(boxes, scores, iou_thres)  # 旋转框nms
        # i = nms_rotated(Rboxes, R_scores, iou_thres)  # 旋转框nms

        #超过max_det=1000个框，只区前1000个
        # if i.shape[0] > max_det:  # limit detections # 水平框nms
        # # if len(i[0]) > max_det:  # limit detections # 旋转框nms
        #     i = i[:max_det]
        # if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)   # flase
        #     # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
        #
        #     iou = box_iou_rotated(boxes[i], boxes, mode='iou') > iou_thres  # iou matrix
        #     # iou = box_iou(boxes[i], boxes) > iou_thres  # iou matrix
        #     weights = iou * scores[None]  # box weights
        ...
        # output[xi] = x[i[1]]   # 旋转框nms

        output[xi] = x_lm[i]    # 水平框nms
        # output[xi] = xxt[i]    # 水平框+旋转nms
        if (time.time() - t) > time_limit:
